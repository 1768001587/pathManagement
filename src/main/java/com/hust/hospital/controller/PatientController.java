...
import com.hust.hospital.util.CheckMedicalNormsUtils;
...
public class PatientController {
    ...
    public Result<List<InPathDto>> getInPaths() {
        ...
        Collections.sort(dtos, new Comparator<InPathDto>() {
            @Override
            public int compare(InPathDto o1, InPathDto o2) {
    ...
    public Result<Map<String, Object>> addPatient(@RequestBody AddPatientDto dto) {
        Map<String,Object> map = new HashMap<>();
        ps.addPatient(new Patient(dto.getName(),dto.getBed(),Status.UNINVOLVED.getId()));
    ...
    public Result<Map<String, Object>> pathSubmit(@RequestBody StageDto dto){
        ...
        if (dto.getWhere() == Stage.ONE.getId()){
            ...
            List cheakList = new ArrayList();
            cheakList.add(ss.getStage1ById(dto.getId()));
            cheakList.add(ss.getStage2ById(dto.getId()));
            cheakList.add(
                    new Stage3(dto.getId(),ch.toJSONString(),ci.toJSONString(),de.toJSONString(),s.getBeginDate(),s.getBeginTime(),s.getEndDate(),s.getEndTime()));
            if(CheckMedicalNormsUtils.checkMedicalNorms(cheakList).getCode()!=200){
                map.put("message",CheckMedicalNormsUtils.checkMedicalNorms(cheakList).getMsg());
                return Result.failed(map);
            }
